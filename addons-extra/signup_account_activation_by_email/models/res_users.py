...
class res_users(models.Model):
    ...
    def _get_activate_url_for_action(self, action=None, view_type=None, menu_id=None, res_id=None, model=None):
        ...
        for partner in self:
            ...
            res[partner.id] = urllib.parse.urljoin(
                base_url, "/web/%s?%s" % (route, werkzeug.url_encode(query)))
        return res
    ...
    def _signup_activate_create_user(self, values):
        ...
        template_portal_user_id = literal_eval(IrConfigParam.get_param('base.template_portal_user_id', 'False'))
        template_user = self.browse(template_portal_user_id)
